...
ui <- fluidPage(
  ...
  #turn this off during testing, turn back on again for deployment
  ...
  titlePanel(title=div(img(src="LOGO NAHRIM_BI.png", style="width: 50px"), 
                       #"Tangki NAHRIM 2.0",
                       HTML("<font size=5>Tangki NAHRIM 2.0</font>   <font size=2.5>v.2.0.2</font>")),
             windowTitle = "Tangki NAHRIM 2.0"),
  
  p("Simple web app to estimate the optimal rainwater harvesting tank size",
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    # app version
    gen_info$version_no <- "Tangki NAHRIM 2.0.2"
  ...
  raindata_sel_df <- reactive({
    ...
    if (is.null(inFile)) {
      ...
      data_source <- "Jabatan Pengairan dan Saliran Malaysia (JPS)"
      
      stn_no_input <- input$stn_Output
      
      list(
        #rainfall data
        raindata_df = raindata_df,
        #data source
        data_source = data_source,
        #station no
        stn_no_input = stn_no_input
      )
      ...
      stn_no_input <- sub('\\.csv$', '', basename(inFile$name)) 
  ...
  rain_summary <- reactive({
    ...
    raindata_sel <- raindata_sel_df()[['raindata_df']]
  ...
  tangki_nahrim <- reactive({
    ...
    for(j in seq(Tank_capacity_1, Tank_capacity_10, Tank_capacity_interval)) {
      ...
      for (i in 2:nrow(raindata)) {
        raindata$yield1[i] = min((raindata$Active_storage[i-1] + raindata$Runoff[i]),
                                 raindata$Tank_capacity[i])
        raindata$Yield[i] = min(c(raindata$yield1[i],
                                  raindata$Water_demand[i]))
        raindata$active_storage1[i] = max(c((raindata$Active_storage[i-1] + raindata$Runoff[i] - raindata$Yield[i]), 
                                            0))
        raindata$active_storage2[i] = max(c((raindata$Tank_capacity[i] - raindata$Yield[i]), 
                                            0))
        raindata$Active_storage[i] = min(c(raindata$active_storage1[i],
                                           raindata$active_storage2[i]))
        raindata$spillage1[i] = (raindata$Active_storage[i-1] + raindata$Runoff[i] - raindata$Tank_capacity[i])
        raindata$Spillage[i] = max(c(raindata$spillage1[i], 
                                     0))
      }
      counter <- counter + 1
    ...
    t_v <- ggplot(t_vol,aes(x = Tank_capacity, y = perc, fill=label_vol,text=hovertext)) +
      ...
      scale_x_continuous(name= "Tank Capacity (m<sup>3</sup>)",
                         breaks = seq(input$tank_cap_min_input, 
                                      ...
                                      by = input$tank_cap_interval_input),
                         minor_breaks = NULL) + #x axis format
      scale_y_continuous(name="Percentage Time (%)",
  ...
  output$downloadResults <- downloadHandler(
    ...
    content = function(file) {
      ...
      input_value <- c(raindata_sel_df()[['stn_no_input']],
                       #input$stn_Output,
      ...
      input_value2 <- c(raindata_sel_df()[['stn_no_input']],
                        #input$stn_Output,
                        rain_summary_val$yr_w_data,
                        rain_summary_val$day_w_data,
                        #lapply(day_norain/day_w_data*100, round, digits = 1),
                        ...
                        input$water_demand_input,
                        paste0(input$tank_cap_min_input, " to ", input$tank_cap_max_input))
      input_value3 <- c(ifelse(is.na(filtered()$STATION_NA), "", filtered()$STATION_NA),
                        #filtered()$STATION_NA,
                        "",
                        "",
                        "",
      ...
      writeData(wb, "Report", paste0("Rainfall data source: ", raindata_sel_df()[['data_source']]),
